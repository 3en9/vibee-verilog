...
class Py2Vibee:
    ...
    def _parse_type_annotation(self, node) -> str:
        ...
        if isinstance(node, ast.Name):
            ...
            # Handle List[T], Dict[K, V], Tuple[T1, T2], etc.
            ...
            # Handle tuple types like Tuple[int, bool]
            return 'Tuple'
    ...
    def _generate_hardware_signals(self, functions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # Basic signals for any hardware module
        signals = [
            {'name': 'clk', 'width': 1, 'direction': 'input'},
            {'name': 'rst_n', 'width': 1, 'direction': 'input'},
            {'name': 'start', 'width': 1, 'direction': 'input'},
            {'name': 'done', 'width': 1, 'direction': 'output'},
            {'name': 'busy', 'width': 1, 'direction': 'output'}
        ]
        
        # For the first function (main function), add its parameters as signals
        if functions:
            ...
            if main_func['returns'] and main_func['returns'] != 'Void':
                signals.append({
                    'name': 'result',
                    'width': self._type_to_width(main_func['returns']),
                    'direction': 'output'
                })
    ...
    def _generate_fsm(self, functions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        # For now, return empty list - FSM generation needs more context
        # In a real implementation, this would analyze control flow in functions
        return []
    ...
    def save_yaml(self, spec: Dict[str, Any], output_path: str):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            # Remove sacred_formula from spec since we write it in header
            spec.pop('sacred_formula', None)
            spec.pop('phoenix', None)
            
            # Write YAML content
            yaml.dump(spec, f, default_flow_style=False, sort_keys=False, width=80, allow_unicode=True)
